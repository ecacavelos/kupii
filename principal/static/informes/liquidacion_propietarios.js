function retrieve_liquidacion_propietarios() {
	...
	$('.monto_otros_descuentos_clase').each(function(index){
		...
		descripcion_descuentos = descripcion_descuentos + "&descripcion_otros_descuentos" + cont + "="  + $(this).parent().parent().find("#descripcion_otros_descuentos" + cont).val();
		cont = cont+1;
	});
	cont = "&cont="+(cont-1);
    window.location.href = base_context + "/informes/liquidacion_propietarios_reporte_excel?fecha_ini=" + $('#fecha_ini').val() + "&fecha_fin=" + $('#fecha_fin').val() + "&tipo_busqueda=" + $('#id_tipo_busqueda').val() + "&busqueda=" + $('#id_busqueda').val()+ "&order_by="+$('[name="order_by"]:checked').val() +"&ley1="+$('#ley1').val()+"&impuesto_renta1="+$('#impuesto_renta1').val()+"&iva_comision="+$('#iva_comision').html()+"&total_a_cobrar="+$('#total_a_cobrar').html()+"&total_descuentos="+$('#total_descuentos').val()+descuentos+descripcion_descuentos+cont;
}
